...
from LeapMotion_detection2 import LeapMotionListener
from pynput.keyboard import Key,KeyCode, Controller
from PIL import Image, ImageTk 
# LIST CMD 
OPTIONS = {'None','Alt','Win',
   'Ctrl','Tab','Shift','Caps','Esc'} # different keys that we can use 
...
DEFAULT_DATA_HEADER_LEFT = "PINKY,RING,MIDDLE,INDEX,TH_PK,TH_RG,TH_MID,TH_IN" # 4 fingers then the treshold for each finger to activate the key
DEFAULT_DATA_HEADER_RIGHT= "INDEX,MIDDLE,RING,PINKY,TH_IN,TH_MID,TH_RG,TH_PK"

class Interface(Tk):  # Tk come from Tkinter and is used to create graphic interface 
    def __init__(self,config_file):

        # READ CONFIG FILE
        self.config_file = config_file
        self.config = configparser.ConfigParser()  # allow us to read INI files
        ...
        #self.labels_buttons = self.config.get("options", "labels_buttons")
        self.labels_buttons = ast.literal_eval(self.config.get("options", "labels_buttons")) #to evaluate the string and convert it 
                                                                                             #convert from unicode to list 
                                                                                             #use ast.literal_eval everytime we have more than one value in config.ini 
        ...
        # DATA RECORDING
        if self.record_data:
            # create file where we will record the distance between the 4 fingers and the thumb : we wrte in this file using : write_in_text_file()
            self.folderData = ".\Data"
            self.folderData2 = ".\Data\hand"
            if not os.path.exists(self.folderData):
                os.makedirs(self.folderData)
            if not os.path.exists(self.folderData2):
                os.makedirs(self.folderData2)
            timestr = time.strftime("%Y%m%d-%H%M%S")
            self.file_object  = open(os.path.join(self.folderData,timestr + '.txt'), "w+")
            self.file_object_hand = open(os.path.join(self.folderData2,timestr + '.txt'), "w+")
        ...
        # READING KEYBOARD
        self.keyboard = Controller()  # come from pynput : The package pynput.keyboard contains classes for controlling and monitoring the keyboard.
                                      # https://pynput.readthedocs.io/en/latest/keyboard.html

        # HAND USE
        self.labels_fingers = self.define_labels_fingers_based_onhand(DEFAULT_LABELS_FINGERS) # depending if left or right hand 
        ...
        self.distance, self.distance_to_palm, self.angle_palm, self.angle_finger_meta,self.angle_finger_prox, self.finger_data = self.read_values_from_devices(self.listener,self.controller)
    ...
    def loading_sound_on_computer(self,OPTIONS):
        # engine = pyttsx3.init(driverName='sapi5') I think this line is not necessary since we don't use engine 
        ...
        for i,opt in enumerate(OPTIONS):
            print(opt)
            theText = opt
            tts = gTTS(text=theText, lang='en') # Writes spoken mp3 data to a file
            tts.save(os.path.join(folderData,theText + ".mp3"))
        print("File saved!")

    def write_in_text_file(self,filehandle,distance,distance_to_palm,angle_palm,angle_finger_meta,angle_finger_prox):
        filehandle.write(",".join(str(item) for item in np.concatenate((distance,distance_to_palm,angle_palm,angle_finger_meta,angle_finger_prox))) + "\n")

    def write_in_text_file2(self,filehandle,finger_data):
        filehandle.write(",".join(str(item) for item in np.concatenate((finger_data['distal'],finger_data['proximal'],finger_data['palm'], finger_data['meta'] ))) + "\n")
    ...
    def move_leap_motion_visualizer(self):
        ...
        # we press the different keys to change the visualizer.exe 
        for i in range(3):
    ...
    def read_values_from_devices(self,listener,controller):
        distance, distance_to_palm, angle_palm, angle_finger_meta, angle_finger_prox, finger_data = listener.on_frame(controller)
        return distance, distance_to_palm, angle_palm, angle_finger_meta, angle_finger_prox, finger_data
    ...
    def run_loop(self):
        ...
        while self._thread is not None:
            ...
            if None in self.distance_to_palm:
                self.calibrated = False
                counter_no_data += 1 
            ...
            if self.record_data:
                ...
                self.write_in_text_file(self.file_object,self.bar_h,self.distance_to_palm,self.angle_palm,self.angle_finger_meta,self.angle_finger_prox)
                self.write_in_text_file2(self.file_object_hand,self.finger_data)
